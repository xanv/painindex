...
For example, consider an article that discusses the painfulness of a 
yellowjacket sting but then goes on to talk about lots of other unrelated stuff.
...
3) A deeper problem is that it doesn't allow us to express more complex
relationships between the appearance of words and predicted pain levels. If the
word "ouch" only appears in tweets with intensity 4, then its mere presence
should predict a 4 regardless of the other words. A properly-designed regression
would take this into account, whereas here we just weight everything equally.
...
def main():
    ...
    # We just use the scraped tweets. There are so few from the API,
    # and it wouldn't be good to use both.

    # with open('../data/outputs/twitter_results_20140910.txt') as json_tweets:
    #     tweets = json.load(json_tweets)
    with open('../data/outputs/twitter_results_scraped_20140910.txt') as json_tweets_scraped:
        tweets = json.load(json_tweets_scraped)
